...
process = kick, rumble, clap, bass, stars, waah, chords, tic :> ef.cubicnl(0,0) <:_,_ ;

//simple
//======

bass = (os.osc(fr)+os.osc(fr*2)*0.6+os.osc(fr*3)*0.3)*gate*0.2*amp *fin with{
...
chords = os.sawtooth(ba.midikey2hz(freq1+12+3.5)),
        ...
        os.sawtooth(ba.midikey2hz(freq3+12+3.5)) :> _*0.8*mute : fi.lowpass(2,600):ef.cubicnl(0,0)*0.1:fi.resonhp(800,3,1) *fin with{
...
tic = ticGen(val)*mute *fin with{
    val = (bar-4)/12:ba.line(200)+rand*0.5;
    rand = no.noise*0.5+0.5:ba.sAndH(ba.pulse(int((60/Tempo/2)*ma.SR))):ba.line(int(60/Tempo/2));
    mute = (loop==2)*(bar>3);
};

waah = waahGen(grog)*mute*0.1 *fin with{
    grog = no.noise*0.5+0.5:ba.sAndH(ba.pulse(int((60/Tempo/2)*ma.SR))):ba.line(int(60/Tempo/2));
    mute = (loop>0)*(bar>7)*(loop%2==1):ba.line(15) + (loop==4)*(bar>7):ba.line(15);
};

stars = simpleSynth(gate,freq)*mute :fx : fi.resonlp(2000,3,0.4) *fin with{
...
kick = kickGen(gate)*mute*1.2 *fin with{
...
rumble = rumbleGen(val)*amp*1.2 with{
    val = ( 100 -((loop>0)|(bar>3))*33.33:ba.line(int(ma.SR*60*16/Tempo)) +5*(bar%4==2)*(loop>1) )*
            (((loop<5)|(bar<8)):ba.line(1000));
    amp = 1:ba.line(int(ma.SR*60*8/Tempo)) -ba.line(15,loop>0) +ba.line(15,loop>1) -ba.line(15,(loop==3)*(bar<8)) -ba.line(1000,(loop>4)*(bar>7)) ;
};

clap = noiseGen(gate,500,3)+@(noiseGen(gate,500,3),del) : _*0.2*mute : fx *fin with{
    gate = (clock!=clock')*ba.selectn(16,clock,list);
